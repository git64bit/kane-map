...
def write_app_real_bundle_config(app_root: Path, data_relative_path: Path) -> None:
    config_path = app_root / "src" / "data" / "realBundleConfig.js"
    config_path.parent.mkdir(parents=True, exist_ok=True)
    data_path = data_relative_path.as_posix()
    config_path.write_text(
        f'''(function attachRealBundleConfig(global) {{
  "use strict";
...
def write_portable_manifest(
    ...
    payload = {
        ...
        "data_path": data_relative_path.as_posix(),
        "source_data_bundle": str(data_bundle),
...
def package_portable_app(
    ...
    if execute:
        if warnings:
            ...
            write_app_real_bundle_config(app_root, data_relative_path)
